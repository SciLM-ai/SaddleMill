...
def nebopt(i, config_dict, traj_name, executorlib_worker_id=None):
    ...
    def log_status(idx, status_msg):
        with open(status_file, 'a') as f:
            f.write(f"{idx},{rank},{status_msg}\n")
    ...
    if converged:
        log_status(i, "converged")
    else:
        log_status(i, "not_converged")
    ...
    max_forces = nebtools.get_fmax(vars(neb))
    ...
    with Trajectory(my_output_file, 'a') as writer:
        ...
        ci_image.info['reactant_positions'] = neb.image[0].positions
        ci_image.info['product_positions'] = neb.image[-1].positions
